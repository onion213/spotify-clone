...
const upsertProductRecord = async (product: Stripe.Product) => {
  ...
  console.debug(`Product inserted/upserted: ${product.id}`)
}
...
const upsertPriceRecord = async (price: Stripe.Price) => {
  ...
  console.debug(`Price inserted/upserted: ${price.id}`)
}

const createOnRetrieveACustomer = async ({
  email,
  uuid,
}: {
  email: string
  ...
  if (error || !data?.stripe_customer_id) {
    ...
    console.debug(`New customer created and inserted for ${uuid}`)
    return customer.id
  }
...
const manageSubscriptionStatusChange = async (
  ...
  console.debug(`Inserted / Upserted subscription [${subscription.id} for ${uuid}]`)
